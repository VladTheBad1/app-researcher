...
### 📋 `project-manager`
- **Purpose**: CCPM workflow specialist for clear project management needs
- **Pattern**: Analyze requirements → Execute CCMP commands → Manage workflows → Delegate services
- **Usage**: When handling features, status, tasks, or structured project workflows
- **Returns**: Command recommendations, workflow status, next steps with user choices

### 🔌 `mcp-handler`
- **Purpose**: Smart MCP gateway for external service operations
- **Pattern**: Detect service → Load specific MCP → Execute operation → Return results
- **Usage**: When handling Firebase, databases, screenshots, transcripts, or API integrations
- **Returns**: Service configuration results and integration status
...
Task: "Add Firebase authentication to my app"
Agent: project-manager
Returns: "PRD created, delegating Firebase setup to mcp-handler"
Main thread never sees: CCMP command details and workflow complexity

# External service integration
Task: "Configure Firebase Auth"
Agent: mcp-handler
Returns: "Firebase Auth configured, config saved to .claude/mcp-config/"
Main thread never sees: MCP installation and configuration details
